...
const addBook = async (req, res) => {
  try {
    ...
    if (!isbn || isbn.length !== 13) {
      return res
        .status(400)
        .json({ success: false, message: "ISBN must be exactly 13 digits." });
    }
  ...
  try {
    ...
    if (!title || !author) {
      return res
        ...
        .json({
          success: false,
          message: "Title and author are required to generate a description.",
        });
    }
...
const chatWithAI = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to process chat message." });
  }
};

export { addBook, getBookDetails, generateDescription, getBooks, chatWithAI };
