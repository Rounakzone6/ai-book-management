...
const BookDetails = () => {
  ...
  const totalPages = Math.ceil(bookList.length / itemsPerPage) || 1;
      ...
      <div className="p-6">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[#4361ee] text-white">
                <th className="py-3.5 px-4 font-semibold text-sm rounded-tl-md">Book ID</th>
                ...
                <th className="py-3.5 px-4 font-semibold text-sm rounded-tr-md text-center">Actions</th>
              </tr>
            </thead>
            <tbody>
              {currentBooks.length > 0 ? (
                currentBooks.map((book, index) => {
                  ...
                  return (
                    <tr key={book._id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                      <td className="py-4 px-4 text-sm text-[#475569]">{globalIndex}</td>
                      <td className="py-4 px-4 text-sm text-[#475569] font-medium">{book.title}</td>
                      <td className="py-4 px-4 text-sm text-[#475569]">{book.author}</td>
                      <td className="py-4 px-4 text-sm text-[#475569]">{book.isbn}</td>
                      <td className="py-4 px-4 text-sm text-[#475569]">{book.price.toFixed(2)}</td>
                      <td className="py-4 px-4 text-sm text-[#475569]">{book.quantity}</td>
                ...
                <tr>
                  <td colSpan="7" className="py-12 text-center text-gray-500 text-sm border-b border-gray-100">
                    No books found. Please add some books from the "Add Book" page.
                  </td>
                </tr>
              )}
            </tbody>
        ...
        {bookList.length > 0 && (
          <div className="flex flex-col items-center justify-center mt-10">
            ...
            <span className="text-sm text-[#64748b] mt-3 font-medium">Page {currentPage} of {totalPages}</span>
          </div>
        )}
      </div>
    </div>
